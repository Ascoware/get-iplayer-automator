...
@objc public class GetCurrentWebpage : NSObject {
    ...
    private class func extractMetadata(url: String, tabTitle: String, pageSource: String, completion: ([Programme]) -> Void) {
        if url.hasPrefix("https://www.bbc.co.uk/iplayer/episode/") {
            ...
            _ = nameScanner.scanString("BBC iPlayer - ")
    ...
    open class func getCurrentWebpage(completion: ([Programme]) -> Void) {
        //Get Default Browser
    ...
    private class func searchForPIDs(url: String) -> [Programme] {
        ...
        task.launchPath = perlBinaryPath.absoluteString
        let args = [
            getIPlayerPath.absoluteString,
            noWarningArg,
            cacheExpiryArg,
            "--pid-recursive-list",
            url,
        ...
        envVariableDictionary["PATH"] = perlEnvironmentPath.absoluteString
        ...
        if let stringData = String(data: data, encoding: .utf8) {
            ...
            for line in lines {
                ...
                program.network = outputParts[1].trimmingCharacters(in: .whitespaces)
